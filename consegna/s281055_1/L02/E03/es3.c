...
int menuParola(comando_e cmd, t_corsa **dataPtr, int *len);

int handleFileInput(char fileName[], t_corsa **dataPtr);
...
comando_e leggiComando(){
    ...
    if(strcmp(word, "6") == 0)              command = i_read;
...
int menuParola(comando_e cmd, t_corsa **dataPtr, int *len){
    t_request req;
    t_corsa corsa;
    t_corsa  *data, *orderCodeData[MAX_LENGTH_DATA], *orderDateData[MAX_LENGTH_DATA], *orderStartData[MAX_LENGTH_DATA], *orderStopData[MAX_LENGTH_DATA];
    int responseList[MAX_LENGTH_DATA], lenResponse, i, delayTot, lenData;
    ...
    switch(cmd){
        ...
        case o_data:                                          break;
        case o_partenza:                                      break;
        case o_arrivo:                                        break;
        case o_codice:                                        break;
        case i_read:            gets(inputFile);              break;
        case r_errore:
    ...
    if(cmd==i_read){
        free(data);
        *len=handleFileInput(inputFile, dataPtr);
...
int handleFileInput(char fileName[], t_corsa **dataPtr){
    ...
    *dataPtr=malloc(lenData*sizeof(t_corsa));
...
int sort(t_request req, t_corsa data[], t_corsa *sorted[], int lenData){
    ...
    for(i=0;i<lenData-1;i++){
        for(j=0;j<lenData-i-1;j++){
            dateSort= req.date==1 && dateCompare(sorted[j]->date,sorted[j+1]->date, t1, t2 );
